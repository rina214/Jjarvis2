...
class Camera2BasicFragment : Fragment(), FragmentCompat.OnRequestPermissionsResultCallback {
  ...
  private fun showToast(text: String) {
    ...
    activity?.runOnUiThread {
      ...
      var position : ArrayList<Pair<Float, Float>> = drawView!!.getPosition()
      if (position.size == 0)
      else if (!(0.3 <= position[1].first) || !(position[1].first <= 0.7))
        tv_notice!!.text = "화면 중앙에 서주세요"
      else if ( !(position[10].second >= 0.8) || !(position[13].second >= 0.8 || !(position[1].second <= 0.2)))
        tv_notice!!.text = "머리는 화면 상단에, 발은 화면 하단에 위치시켜 주세요"
      else {
        if (position[8].second <= 0.5 && position[11].second <= 0.5) {
          tv_notice!!.text = "스쿼트 자세로 앉아주세요."
          currentState = false
        } else if (position[8].second >= 0.55 && position[11].second >= 0.55) {
          tv_notice!!.text = "스쿼트 자세로 일어나주세요."
          currentState = true
        }
...
//      }
  ...
  private fun allPermissionsGranted(): Boolean {
    //for (permission in requiredPermissions) {
      if (Build.VERSION.SDK_INT >= 23 && ContextCompat.checkSelfPermission(context, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
        return false
      }
    //}
    return true
